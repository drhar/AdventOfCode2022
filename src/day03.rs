...
pub fn day03(input_lines: &str) -> (String, String) {
    ...
    let answer2: i32 = input_lines.lines().chunks(3).into_iter().map(|group| {
        let Some(elf_group): Option<(String, String, String)> = group.map(|line| {
        ...
        elf_group.0
            ...
            .into_iter()
            .unique()
            .map(|item| priorities.get(&item).unwrap()).sum::<i32>()
    }).sum::<i32>();
...
ttgJtRGJQctTZtZT
CrZsJsPPZsGzwwsLwLmpwMDw").0, "157".to_string())
    }
...
wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn
ttgJtRGJQctTZtZT
CrZsJsPPZsGzwwsLwLmpwMDw"), ("157".to_string(), "70".to_string()))
    }
}
